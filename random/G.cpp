...
int main() {
    ...
    */ 
    ...
    /*
    for (auto [x, y]: match) cerr << x << ' ' << y << ' '; 
    cerr << '\n'; 
    */ 

    vector<ii> ans; 
    for (auto s: sources) if (done[s] == 0) ans.pb({match[0].ss, s}); 
    for (auto t: sinks) if (done[t] == 0) ans.pb({t, match[0].ff}); 
