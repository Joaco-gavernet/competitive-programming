...
#define r(x) int(x<<1|1)
  ...
  int at_least(int x, int p = 1) {
    auto &cur = t[p]; 

    if (cur.ans < x) return INF; 
    if (cur.l == cur.r and x <= cur.ans) return cur.l; 

    auto &curl = t[p<<1]; 
    auto &curr = t[p<<1|1]; 

    if (x <= curl.ans) return at_least(x, p<<1);
    else return at_least(x, p<<1|1); 
  } 
};
