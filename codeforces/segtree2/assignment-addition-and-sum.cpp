...
struct node {
  tipo ans, l, r, lset, lsum;
  bool upd;
  node() { ans = lsum = 0; lset = -1; upd = false; l = r = -1; } // REMINDER !!! SET NEUT
  node(tipo val, int pos) : ans(val), l(pos), r(pos), lsum(0), lset(-1), upd(false) {} // Set node
  void set_lazy(tipo _set, tipo _sum) {
    if (_set != -1) {
      lsum = _sum;
      lset = _set;
    } else lsum += _sum;
...
struct segtree_lazy {
  ...
  void node_update(node &cur) {
    if (cur.lset != -1) cur.ans = cur.lset *(min(n, cur.r) -cur.l +1);
    cur.ans += cur.lsum *(min(n, cur.r) -cur.l +1); //Operacion update
  }
  ...
  void push(int p) {
    ...
    if(cur.upd == true) {
      node_update(cur);
      if(cur.l < cur.r) {
        t[l(p)].set_lazy(cur.lset, cur.lsum);
        t[r(p)].set_lazy(cur.lset, cur.lsum);
      }
      reset_lazy(cur);
    }
  }
  ...
  void update(int l, int r, tipo _set, tipo _sum, int p = 1) { // root at p = 1
    push(p); node &cur = t[p];
    if(l > cur.r || r < cur.l) return;
    if(l <= cur.l && cur.r <= r) {
      cur.set_lazy(_set, _sum); push(p); return;
    }
    update(l, r, _set, _sum, l(p)); update(l, r, _set, _sum, r(p));
    cur = op(t[l(p)], t[r(p)]);
  }
