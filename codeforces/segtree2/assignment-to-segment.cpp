...
struct node {
  tipo ans, l, r, lazy;
  bool upd;
  node() { ans = 0; lazy = -1; upd = false; l = r = -1; } // REMINDER !!! SET NEUT
  node(tipo val, int pos) : ans(val), l(pos), r(pos), lazy(-1), upd(false) {} // Set node
...
struct segtree_lazy {
  ...
  void node_update(node &cur) {
    if (cur.upd) cur.ans = cur.lazy; //Operacion update
  }

  void reset_lazy(node &cur) {
    cur.lazy = -1; cur.upd = false; //Poner el neutro del update
  }
