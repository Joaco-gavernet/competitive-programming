...
struct node {
  tipo ans, l, r, lazy;
  bool upd;
  node() { ans = INF; lazy = INF; upd = false; l = r = -1; } // REMINDER !!! SET NEUT
  node(tipo val, int pos) : ans(val), l(pos), r(pos), lazy(INF), upd(false) {} // Set node
...
struct segtree_lazy {
  ...
  node op(node a, node b) { // Operacion de query
    node aux; 
    aux.ans = min(a.ans, b.ans);
    aux.l = a.l; aux.r = b.r;
    return aux;
  }

  void node_update(node &cur) { //Operacion update
    if (cur.upd) cur.ans = cur.lazy; 
  }
